...
public class ParquetUtil {
	...
	GenericRecord convertToAvro(Resource resource) {
		// TODO update Bunsen and get rid of this R3 conversion. Note that currently the conversaion
		// can fail for some R4 resource types (and they should be excluded).
		org.hl7.fhir.dstu3.model.Resource r3Resource = org.hl7.fhir.convertors.VersionConvertor_30_40
		        .convertResource(resource, true);
		if (r3Resource == null) {
			log.error("R3 conversion failed; ignorning resource with type " + resource.getResourceType().name());
			return null;
		}
